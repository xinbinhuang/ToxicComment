...
tfidf_vectorizer=CountVectorizer(encoding="unicode", 
                analyzer="word",
                token_pattern="\w{1,}",
                stop_words="english",
                ngram_range=(1,1),
...
train_tfidf_features=tfidf_vectorizer.transform(train_text)
...
for class_name in class_names:
    ...
    cv_score = np.mean(cross_val_score(classifier, train_tfidf_features, train_target, cv=3, scoring="roc_auc"))
    scores.append(cv_score)
    print("Average CV score for {} : {}".format(class_name, cv_score))
    
    classifier.fit(train_tfidf_features, train_target)
    y_pred_tfidf[class_name]=classifier.predict_proba(test_tfidf_features)[:, 1]
